...
// $Producto,
    escribirRegistroVentas(4, contador-1, "                                         $", "", campos, outputFile);
...
outputFile << "    public function eliminar"+nombreModelo+"Por"+campos[3]+"($"+campos[3]+"){" << endl;

outputFile << "        $query = `DELETE FROM "+nombreModelo+" WHERE "+campos[3]+" = :"+campos[3]+" `;" << endl;
outputFile << "        $stmt = $this->conexion->prepare($query);" << endl;
outputFile << "        $stmt->bindParam(\':"+campos[3]+"\', $"+campos[3]+", PDO::PARAM_INT);" << endl;
outputFile << "" << endl;
outputFile << "        return $stmt->execute();" << endl;
outputFile << "    }" << endl;
outputFile << "" << endl;
...
outputFile << "    public function actualizar"+nombreModelo+"(" << endl;
 // $id, $producto, $cantidad, $descripcion, $total, $fecha
    escribirRegistroVentas(3, contador-1, "                                         $", "", campos, outputFile);
outputFile << " )" << endl;
outputFile << "    {" << endl;
...
outputFile << "                  WHERE "+campos[3]+" = :"+campos[3]+"`;" << endl;
outputFile << "        $stmt = $this->conexion->prepare($query);" << endl;
outputFile << "" << endl;
...
outputFile << "    }" << endl;
outputFile << "" << endl;
outputFile << "    // Método para obtener una venta por su "+campos[3] << endl;
outputFile << "    public function obtener"+nombreModelo+"PorId($"+campos[3]+")" << endl;
outputFile << "    {" << endl;
outputFile << "        $query = `SELECT " << endl;
//id, producto, cantidad, descripcion, total, fecha 
    escribirRegistroVentas(3, contador-1, "              ", "", campos, outputFile);

outputFile << " FROM ventas WHERE "+campos[3]+" = :"+campos[3]+"`;"<< endl;
outputFile << "        $stmt = $this->conexion->prepare($query);" << endl;
outputFile << "        $stmt->bindParam(\':"+campos[3]+"\', $"+campos[3]+", PDO::PARAM_INT);" << endl;
